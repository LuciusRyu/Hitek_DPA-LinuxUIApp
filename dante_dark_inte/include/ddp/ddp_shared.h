...
/*
Removes last element from the array. Sets the offset to the last element to zero in the array 
and decrements the count.

Returns error if there are no elements allocated in the array.
*/
...
/*
	Dereference an element of an offset array and check size dynamically.

	The first uint16_t of the dereferenced memory block contains the length (in bytes) of that block.
	A min_size < 2 (e.g. 0) will be treated as 2 (the two byte size field is mandatory).

	Returns NULL if
	array index is invalid
	size field < min_size
	size does not fit within message bounds.
	Otherwise returns pointer to memory
*/
