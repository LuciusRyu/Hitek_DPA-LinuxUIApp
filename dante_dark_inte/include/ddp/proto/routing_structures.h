...
enum ddp_routing_rx_flow_flags
{
	...
	DDP_ROUTING_RXFLOW_FLAG__FLOWSUB = 1 << 2,	 //!< Indicates that this Rx flow uses flow subscription behaviour
};
...
/**
* @enum routing_flow_data_extension_flags
* @brief Type of the extension that transport_extension_offset points to.
*/
enum {
	ROUTING_FLOW_DATA_EXTENSION_FLAG__TRANSPORT = 1 << 0		//!< 'transport' extension exists.
};
...
struct ddp_routing_flow_data
{
	...
	uint8_t transport;					/*!< Audio transport protocol, @see ddp_audio_transport for supported protocols*/
	uint8_t extension_flags;			/*!< Flags for extensions @see routing_flow_data_extension_flags  */
	...
	// Extensions
	ddp_raw_offset_t transport_extension_offset;	/*!< Offset for extension of transport details of flow data. Available when extension_flags has a valid non-zero flag */
	uint16_t transport_extension_size;		/*!< size of the transport extension in bytes. */
};
