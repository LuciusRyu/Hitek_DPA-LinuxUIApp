...
/**
 * @enum clock_castness
 * @brief Clock port castness preferred values for ddp_clock_port_raw
 */
enum clock_castness
{
 ...
 * @brief Clock port state values for ddp_clock_port_raw
 */
enum clock_port_state
{
 ...
 * @brief Clock port bit flags for ddp_clock_port_raw
 */
enum clock_port_flags
{
 ...
 * @brief Clock port response flags for ddp_clock_port_raw
 */
enum clock_config_port_response_valid_flags
{
...
/**
 * @enum clock_basic2_valid_flags
 * Valid flags for some of the fields in the response message.
 */
enum clock_basic2_valid_flags{
	DDP_CLOCK_BASIC2_VALID_FLAG__V2_DOMAIN_NUMBER = 0x1 << 0,  /*!< v2 domain number field is valid */
	DDP_CLOCK_BASIC2_VALID_FLAG__PORT_RAW_ARRAY = 0x1 << 1,    /*!< port raw struct offset and size fields are valid */
	DDP_CLOCK_BASIC2_VALID_FLAG__V2_UUID_FORMAT = 0x1 << 2     /*!< message contain explicit v1 and v2 uuids */
};

/**
 * @enum ddp_clock_port_raw_valid_flags
 * valid flags for the port fields in the ddp_clock_port_raw
 */
enum ddp_clock_port_raw_valid_flags {
	DDP_CLOCK_PORT_RAW_VALID_FLAG__FLAGS = 0x1 << 0,		/*!< flags field is valid in ddp_clock_port_raw */
	DDP_CLOCK_PORT_RAW_VALID_FLAG__PORT_ID = 0x1 << 1,		/*!< port_id field is valid in ddp_clock_port_raw */
	DDP_CLOCK_PORT_RAW_VALID_FLAG__PROTOCOL = 0x1 << 2,		/*!< protocol field is valid in ddp_clock_port_raw */
	DDP_CLOCK_PORT_RAW_VALID_FLAG__CASTNESS = 0x1 << 3,		/*!< castness field is valid in ddp_clock_port_raw */
	DDP_CLOCK_PORT_RAW_VALID_FLAG__INTERFACE_INDEX = 0x1 << 4, /*!< interface_index field is valid in ddp_clock_port_raw */
	DDP_CLOCK_PORT_RAW_VALID_FLAG__STATE = 0x1 << 5,		/*!< port_state field is valid in ddp_clock_port_raw */
	DDP_CLOCK_PORT_RAW_VALID_FLAG__NETWORK_INDEX = 0x1 << 6 /*!< network_index field is valid in ddp_clock_port_raw */
};
 ...
 * @deprecated Use \ref DDP_V2_CLOCK_UUID_LENGTH instead
 */
#define DDP_CLOCK_UUID_LENGTH 6

/**
 * The length of PTP v2 based Audinate clock ID
 */
#define DDP_V2_CLOCK_UUID_LENGTH 8
 ...
 * @deprecated - use ddp_v2_clock_uuid instead
 */
struct ddp_clock_uuid
{
...
/* Defintion
 * @brief Structure format for the ddp clock UUID's based on Ptp v1
 */
typedef struct ddp_clock_uuid ddp_clock_uuid_t;
//! @cond

/**
 * @struct ddp_v2_clock_uuid
 * @brief Structure format for the ddp clock UUID's based on Ptp v2
 */
struct ddp_v2_clock_uuid
{
	uint8_t data[DDP_V2_CLOCK_UUID_LENGTH];	/*!< The UUID */
};

//! @cond Doxygen_Suppress
typedef struct ddp_v2_clock_uuid ddp_v2_clock_uuid_t;
//! @endcond

/**
 * @struct ddp_clock_port
 * @brief Structure format for each clock port message
 * @deprecated: User \ref ddp_clock_port_raw_t instead; Only here for reading old apis on client.
 */
struct ddp_clock_port
{
...
/**
 * @struct ddp_clock_port_raw
 * @brief Structure format for each clock port updated message
 */
struct ddp_clock_port_raw
{
	uint16_t flags;				/*!< per port bit flags @see clock_port_flags */
	...
	uint8_t interface_index;	/*!< port interface index */
	uint8_t port_state;			/*!< port state @see clock_port_state */
	uint16_t valid_flags;		/*!< valid field flags as noted in enum ddp_clock_port_raw_valid_flags */
	uint16_t network_index;		/*!< port network index @see enum network_intf_index in network_structures.h */
};

//! @cond Doxygen_Suppress
typedef struct ddp_clock_port_raw ddp_clock_port_raw_t;
//! @endcond
...
struct ddp_clock_basic2_response
{
	...
	struct ddp_clock_basic2_response__payload
	{
		...
		uint8_t v2_domain_number;					/*!< Domain number for ptp v2 clock - ONLY valid when  CLOCK_BASIC2_VALID_FLAG__V2_DOMAIN_NUMBER is set*/
		int32_t drift;								/*!< Current clock drift in parts per million (PPM) */
		int32_t max_drift;							/*!< Maximum clock drift in parts per million (PPM) */
		...
		uint16_t valid_flags;						/*!< Valid flags for some of the fields in the response message - see  clock_basic2_valid_flags */
		uint16_t port_raw_size;						/*!< Size of each ptp raw port structure @see ddp_clock_port_raw @see port_raw_array_offset */
		ddp_raw_offset_t port_raw_array_offset;		/*!< Offset from the start of the TLV0 header to the first clock raw port structure @see ddp_clock_port_raw */
