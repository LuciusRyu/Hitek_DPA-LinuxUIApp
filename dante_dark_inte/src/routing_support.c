...
#include "../include/ddp/proto/audio_structures.h"
...
)
{
	...
	data = (ddp_routing_rx_channel_data_t *)ddp_offset_array_read_element_self_sized(&heap, chan_idx, element_size);
	if (data)
	{
...
)
{
	...
	data = (ddp_routing_tx_channel_data_t *)ddp_offset_array_read_element_self_sized(&heap, chan_idx, element_size);
	if (data)
	{
...
// return success if fd is a valid pointer
aud_error_t
ddp_get_raw_flow_state_data_ptr
(
	const ddp_message_read_info_t * message_info,
	uint16_t flow_idx,
	ddp_routing_flow_data_t ** pfd
)
{
	aud_error_t result;
	ddp_heap_read_info_t heap;
	uint16_t num_flows, min_size;
	...
	min_size = offsetof(ddp_routing_flow_data_t, transport_extension_offset);
	...
	*pfd = (ddp_routing_flow_data_t *)ddp_offset_array_read_element_self_sized(&heap, flow_idx, min_size);
	if (!*pfd)
		return AUD_ERR_NODATA;
	return AUD_SUCCESS;
}
...
/*		element_size = ntohs(fd->size);
		if (element_size > AUD_OFFSETOF_END(*fd, transport_extension_offset))
		{
			//populate extension here
		}*/
	return AUD_SUCCESS;
}
