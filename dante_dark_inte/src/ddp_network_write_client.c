...
)
{
	...
	result = ddp_add_network_config_request_nw_ip(packet_info, request_id, intf_index, mode, &ip_config_nw, &dns_config_nw, dns_domain_name);
	...
	)
{
	...
	if(dns_domain_name)
	{
		if (!
			ddp_message_allocate_string(
				&message_info,
				&raw_header->payload.dns_domain_offset,
				dns_domain_name
		))
		{
	...
	{
		raw_header->payload.dns_domain_offset = 0;
	}
