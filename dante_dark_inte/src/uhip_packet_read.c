...
#include "../include/uhip_packet_read.h"
...
aud_error_t
uhip_packet_read_device_config_data
(
	uhip_packet_read_info_t * packet_info,
	uint32_t * out_seq_num,
	uint32_t * out_crc_checksum,
	uint8_t ** out_payload,
	uint16_t * out_payload_length
)
{
	...
	if ((packet_info->_.header->packet_type != UhipPacketType_DEVICE_CONFIG_DATA) ||
		(packet_info->packet_length_bytes < sizeof(uhip_device_config_data_t)))
	{
		return AUD_ERR_INVALIDDATA;
	}

	uhip_device_config_data_t* device_config_data_packet = (uhip_device_config_data_t*)packet_info->_.buf32;

	if (out_seq_num)
	{
		*out_seq_num = (uint32_t)(ntohl(device_config_data_packet->seq_num));
	}

	if (out_crc_checksum)
	{
		*out_crc_checksum = (uint32_t)(ntohl(device_config_data_packet->crc_value));
	}
