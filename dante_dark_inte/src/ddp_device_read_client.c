...
)
{
	...
	//get the Dante domain name
	if (out_dante_domain_name)
	{
		if (valid_flags & DEVICE_DANTE_DOMAIN_RESPONSE_INDEX_DANTE_DOMAIN_NAME_VALID)
		{
			ddp_raw_offset_t dante_domain_name_offset = ntohs(header->payload.dante_domain_name_offset);
			if (dante_domain_name_offset)
			{
				*out_dante_domain_name = (char *)header + dante_domain_name_offset;
			}
		}
	}

	//get the Dante domain UUID
	if (out_dante_domain_uuid)
	{
		if (valid_flags & DEVICE_DANTE_DOMAIN_RESPONSE_INDEX_DANTE_DOMAIN_UUID_VALID)
		{
			ddp_raw_offset_t dante_domain_uuid_offset = ntohs(header->payload.dante_domain_uuid_offset);
			if (dante_domain_uuid_offset)
			{
				*out_dante_domain_uuid = (ddp_dante_domain_uuid_t *)((uint8_t *)header + dante_domain_uuid_offset);
			}
		}
	}
...
)
{
	aud_error_t result;
	ddp_message_read_info_t message_info;
	ddp_device_config_rev_num_response_t * header;
	...
	result = ddp_packet_validate_message(&message_info, DDP_OP_DEVICE_CONFIG_REVISION, sizeof(ddp_device_config_rev_num_response_t));
	if (result != AUD_SUCCESS)
	{
		return result;
	}

	header = (ddp_device_config_rev_num_response_t *)message_info.header.header;
	*rev_num = ntohl(header->payload.revision_num);
...
//	uint16_t num_of_packets;
	aud_error_t result;
	ddp_message_read_info_t message_info;
	ddp_device_config_read_response_t * header;
	...
	result = ddp_packet_validate_message(&message_info, DDP_OP_DEVICE_READ_CONFIG, sizeof(ddp_device_config_read_response_t));
	if (result != AUD_SUCCESS)
	{
		return result;
	}

	header = (ddp_device_config_read_response_t *)message_info.header.header;

	*num_packets = ntohs(header->payload.num_of_packets);
...
)
{
	...
	result = ddp_packet_validate_message(&message_info, DDP_OP_DEVICE_WRITE_CONFIG, sizeof(ddp_device_config_write_response_t));
	if (result != AUD_SUCCESS)
	{
		return result;
	}
...
)
{
	...
	aud_error_t result;
	ddp_message_read_info_t message_info;
	ddp_device_config_pkt_ack_nack_t * header;
	...
	result = ddp_packet_validate_message(&message_info, DDP_OP_DEVICE_CONFIG_PKT_ACK, sizeof(ddp_device_config_pkt_ack_nack_t));
	if (result != AUD_SUCCESS)
	{
		return result;
	}

	header = (ddp_device_config_pkt_ack_nack_t *)message_info.header.header;

	*seq_num = ntohl(header->payload.sequence_number);
...
)
{
	...
	result = ddp_packet_validate_message(&message_info, DDP_OP_DEVICE_CONFIG_PKT_NACK, sizeof(ddp_device_config_pkt_ack_nack_t));
	if (result != AUD_SUCCESS)
	{
		return result;
	}
...
)
{
	...
	aud_error_t result;
	ddp_message_read_info_t message_info;
	ddp_device_peer_group_id_response_t * header;
	...
	result = ddp_packet_validate_message(&message_info, DDP_OP_DEVICE_PEER_GROUP_ID, sizeof(ddp_device_peer_group_id_response_t));
	if (result != AUD_SUCCESS)
	{
		return result;
	}

	header = (ddp_device_peer_group_id_response_t *)message_info.header.header;

	if (peer_group_id) {
		memcpy(peer_group_id, header->payload.peer_group_id, sizeof(ddp_dante_domain_uuid_t));
	}
...
)
{
	...
	result = ddp_packet_validate_message(&message_info, DDP_OP_DEVICE_CONFIG_END_BACKUP, sizeof(ddp_device_config_end_backup_response_t));
	if (result != AUD_SUCCESS)
	{
		return result;
	}
...
)
{
	...
	result = ddp_packet_validate_message(&message_info, DDP_OP_DEVICE_CONFIG_END_RESTORE, sizeof(ddp_device_config_end_restore_response_t));
	if (result != AUD_SUCCESS)
	{
		return result;
	}
