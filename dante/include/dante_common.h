...
enum dante_rxstatus
{
	...
	DANTE_RXSTATUS_CHANNEL_FORMAT = 0x10,

	  /** Error: Flow formats do not match, 
          e.g. Multicast flow with more slots than receiving device can handle */
	DANTE_RXSTATUS_BUNDLE_FORMAT = 0x11,

	  /** Error: Receiver is out of resources (e.g. flows) */
	DANTE_RXSTATUS_NO_RX = 0x12,

	  /** Error: Receiver couldn't set up the flow */
	DANTE_RXSTATUS_RX_FAIL = 0x13,

	  /** Error: Transmitter is out of resources (e.g. flows) */
	DANTE_RXSTATUS_NO_TX = 0x14,

	  /** Error: Transmitter couldn't set up the flow */
	DANTE_RXSTATUS_TX_FAIL = 0x15,

	  /** Error: Receiver got a QoS failure (too much data) when setting up the flow. */
	DANTE_RXSTATUS_QOS_FAIL_RX = 0x16,

	  /** Error: Transmitter got a QoS failure (too much data) when setting up the flow. */
	DANTE_RXSTATUS_QOS_FAIL_TX = 0x17,

	  /** Error: TX rejected the address given by rx (usually indicates an arp failure) */
	DANTE_RXSTATUS_TX_REJECTED_ADDR = 0x18,

	  /** Error: Transmitter rejected the bundle request as invalid */
	DANTE_RXSTATUS_INVALID_MSG = 0x19,

	  /** Error: TX channel latency higher than maximum supported RX latency */
	DANTE_RXSTATUS_CHANNEL_LATENCY = 0x1A,

	  /** Error: TX and RX and in different clock subdomains */
	DANTE_RXSTATUS_CLOCK_DOMAIN = 0x1B,

	  /** Error: Attempt to use an unsupported feature */
	DANTE_RXSTATUS_UNSUPPORTED = 0x1C,

	  /** Error: All rx links are down */
	DANTE_RXSTATUS_RX_LINK_DOWN = 0x1D,

	  /** Error: All tx links are down */
	DANTE_RXSTATUS_TX_LINK_DOWN = 0x1E,

	  /** Error: can't find suitable protocol for dynamic connection */
	DANTE_RXSTATUS_DYNAMIC_PROTOCOL = 0x1F,

	  /** Channel does not exist (eg no such local channel) */
	DANTE_RXSTATUS_INVALID_CHANNEL = 0x20,
		
	  /** Tx Scheduler failure */
	DANTE_RXSTATUS_TX_SCHEDULER_FAILURE = 0x21,

	  /** The given subscription to self was disallowed by the device */
	DANTE_RXSTATUS_SUBSCRIBE_SELF_POLICY = 0x22,

	  /** Discoverted TX channel is not ready */
	DANTE_RXSTATUS_TX_NOT_READY = 0x23,

	  /** receive channel not ready*/
	DANTE_RXSTATUS_RX_NOT_READY = 0x24,

	  /** TX channel has reached fan out limt */
	DANTE_RXSTATUS_TX_FANOUT_LIMIT_REACHED = 0x25,

	  /** Tx channel is encrypted.*/
	DANTE_RXSTATUS_TX_CHANNEL_ENCRYPTED = 0x26,

		/** Unexpected response from Tx*/
	DANTE_RXSTATUS_TX_RESPONSE_UNEXPECTED = 0x27,
	...
	DANTE_RXSTATUS_TEMPLATE_MISMATCH_FORMAT = 0x41,

	  /** Template-based subscription failed: the channel is not part of the given multicast flow */
	DANTE_RXSTATUS_TEMPLATE_MISSING_CHANNEL = 0x42,
	...
	DANTE_RXSTATUS_TX_ACCESS_CONTROL_PENDING = 0x61,

	  /*Rx flow has HDCP key negotiation error*/
	DANTE_RXSTATUS_HDCP_NEGOTIATION_ERROR = 0x70,
...
/**
* Compare two single 'formats' structures
*
* @param f1 the first format
* @param f2 the second format
*
* @return AUD_TRUE if the 'format' structures are equal, AUD_FALSE otherwise
*/
AUD_INLINE aud_bool_t
dante_format_equals
(
	...
	return f1->samplerate == f2->samplerate && f1->encoding == f2->encoding;
}
...
{	
	...
	fd_set write_fds;
	  /** The 'except' fd set for a 'select' call */
	fd_set except_fds;
} dante_sockets_t;
...
/**
 * Helper method to add a except socket to the socket set, by adding s to except_fds
 * and updating 'n'. The behaviour this function is platform dependent.
 * On a posix platform, the new value for 'n' is the maximum of its old
 * value and (s+1). On a windows platform, n is incremented by 1 (though the value is ignored by select())
 *
...
#pragma warning(push)
#pragma warning(disable:4127)
	if (!FD_ISSET(s, &sockets->except_fds))
	{
		FD_SET(s, &sockets->except_fds);
		sockets->n++;
	}
#pragma warning(pop)
#else
 ...
 * On a windows platform, n is decremented by 1 (though the value is ignored by select())
 *
 * @param sockets the socket set to which the socket will be removed
...
/**
 * Helper method to remove a except socket from a socket set, by removing s from except_fds
 * and updating 'n'. The behaviour this function is platform dependent.
 * On a posix platform, the new value for 'n' is the value +1 of the new maximum fd.
 * On a windows platform, n is decremented by 1 (though the value is ignored by select())
 *
...
#pragma warning(pop)
	...
	if (sockets->n == s+1)
	{
		...
		for (i = 0; i < s; i++)
		{
			if (FD_ISSET(i, &sockets->except_fds))
			{
				sockets->n = i+1;
			}
		}
...
(
	...
	AUD_FD_COPY(&src->except_fds, &dst->except_fds);
}
