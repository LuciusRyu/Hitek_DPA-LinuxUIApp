const PentaVolumeSlide = class penta_volume_slide {
    ...
    _resetAll() {
        ...
        let html = `
            <div class="relative w-full h-full">
                <div class="absolute top-0 w-[20px] h-full transform -translate-x-1/2 left-1/2 border-0 rounded-[22px] bg-gradient-to-b from-red-500 via-yellow-400 to-green-500"></div>
                <div id="${this.fillID}" class="absolute top-0 w-[20px] h-full transform -translate-x-1/2 left-1/2 border-0 rounded-[22px] bg-black"></div>       
                <div
                    id="${this.btnID}" 
                    class="absolute inset-x-0 bottom-0 border-0 left-1/2 transform -translate-x-1/2 w-full h-[32px] rounded-[32px] bg-[#7f8ba8] m-0 text-center cursor-pointer text-[12px] leading-[24px]"
                >
                    <div class="flex items-center justify-center w-full h-full">${btnV}</div>
                </div>                   
            </div>
        `;
