...
const ConnectionManager = class connection_manager {
    ...
    on_NativeCall(jsV) {
        if (jsV.act == "GET_BASEINFO_RES") {
            ...
            if (jsV.payload == "NONE") {
                ...
                else if (sysInfo.status == "ING") { //아직 진행중... 0.5초 후 재시도
                    setTimeout(function() {
                        this._callNative("GET_SYS_CONFIG", "NONE");
                    }.bind(this), 500);
                }
                else alert('시스템 오류가 발생하였습니다: ' + sysInfo.error);
            }
    ...
    */
    ...
    _onNewServer(seq, groupList) {
        ...
        let newMTX = new MainTXConnector(this.dante_dev_id, this.baseInfo, this.checkNew.list[this.checkNew.counter], groupList, this.mtx_seq++);
